...
const LeadsPage = {
  ...
  async render() {
    document.getElementById('page-title').textContent = 'Mine Leads';
    document.getElementById('page-subtitle').textContent = 'Mining engine with real APIs from Receita Federal and OpenStreetMap';
    ...
    document.getElementById('page-leads').innerHTML = `
      <div class="service-card">
        ...
        <div class="service-card-header">
          ...
          <div class="service-card-info">
            <h3 class="service-card-title">Power Mining Engine</h3>
            <p class="service-card-subtitle">BrasilAPI (Receita Federal) + OpenStreetMap Nominatim + ViaCEP</p>
          </div>
          <span class="service-card-badge badge-live">Real API</span>
        </div>
          ...
          <form id="mine-form" onsubmit="LeadsPage.mine(event)">
            <div style="display:grid;grid-template-columns:1.5fr 1fr auto;gap:1rem;align-items:end;">
              <div class="form-group">
                ...
                <input type="text" id="mine-keyword" placeholder="Ex: Restaurant, Clinic, Real Estate, Bakery..." required>
              </div>
              <div class="form-group">
                <label>City / State</label>
                <input type="text" id="mine-city" placeholder="Ex: Curitiba PR, Sao Paulo SP..." required>
              </div>
              <button type="submit" class="btn btn-primary" id="btn-mine"><i data-lucide="radar"></i>Mine</button>
            </div>
          </form>
        </div>
      </div>
      ...
      <div class="service-card">
        ...
        <div class="service-card-header">
          ...
          <div class="service-card-info">
            <h3 class="service-card-title">Mine Individuals</h3>
            <p class="service-card-subtitle">Generate contacts for professionals, service providers, and merchants by city</p>
          </div>
          <span class="service-card-badge badge-ai">AI</span>
        </div>
        <div class="service-card-body">
          <div id="pf-category-tabs" class="pf-tabs">
            <button class="pf-tab active" onclick="LeadsPage.setPFCategory('profissionais')" id="pf-tab-profissionais"><i data-lucide="briefcase"></i>Professionals</button>
            <button class="pf-tab" onclick="LeadsPage.setPFCategory('prestadores')" id="pf-tab-prestadores"><i data-lucide="wrench"></i>Service Providers</button>
            <button class="pf-tab" onclick="LeadsPage.setPFCategory('comerciantes')" id="pf-tab-comerciantes"><i data-lucide="store"></i>Merchants</button>
            <button class="pf-tab" onclick="LeadsPage.setPFCategory('todos')" id="pf-tab-todos"><i data-lucide="layers"></i>All</button>
          </div>
          <form id="pf-mine-form" onsubmit="LeadsPage.mineIndividuals(event)">
            <div style="display:grid;grid-template-columns:1.5fr auto auto;gap:1rem;align-items:end;">
              ...
              <button type="submit" class="btn btn-pink" id="btn-pf-mine"><i data-lucide="user-plus"></i>Mine Individuals</button>
            </div>
          </form>
      ...
      <div class="service-card">
        ...
        <div class="service-card-header">
          ...
          <div class="service-card-info">
            <h3 class="service-card-title">Real CNPJ Lookup</h3>
            <p class="service-card-subtitle">Data directly from Receita Federal via BrasilAPI</p>
          </div>
          <span class="service-card-badge badge-federal">Federal Revenue</span>
        </div>
        <div class="service-card-body">
          <div style="display:flex;gap:1rem;align-items:end;">
            ...
            <button class="btn btn-secondary" onclick="LeadsPage.lookupCNPJ()"><i data-lucide="search"></i>Lookup</button>
          </div>
      ...
      <div class="service-card">
        ...
        <div class="service-card-header">
          ...
          <div class="service-card-info">
            <h3 class="service-card-title">CPF Validator</h3>
            <p class="service-card-subtitle">Complete mathematical validation + personal data estimation</p>
          </div>
          <span class="service-card-badge badge-free">Free</span>
        </div>
        <div class="service-card-body">
          <div style="display:flex;gap:1rem;align-items:end;">
            <div class="form-group" style="flex:1;">
            ...
            <button class="btn btn-secondary" onclick="LeadsPage.lookupCPF()" id="btn-cpf-lookup"><i data-lucide="search"></i>Lookup</button>
          </div>
      ...
      <div class="service-card">
        ...
        <div class="service-card-header">
          ...
          <div class="service-card-info">
            <h3 class="service-card-title">Real People via CNPJ</h3>
            <p class="service-card-subtitle">Search CNPJs and extract real partners from Receita Federal</p>
          </div>
          <span class="service-card-badge badge-federal">Federal Revenue</span>
        </div>
        <div class="service-card-body">
          <div style="margin-bottom:0.75rem;">
            <label style="font-size:0.8rem;color:var(--text-secondary);margin-bottom:4px;display:block;">Paste CNPJs (one per line or separated by comma):</label>
          ...
          <div style="display:flex;gap:0.75rem;align-items:center;">
            <button class="btn btn-primary" onclick="LeadsPage.minePeople()" id="btn-mine-people">
              <i data-lucide="radar"></i>Extract Real People
            </button>
      ...
      <div class="service-card">
        ...
        <div class="service-card-header">
          ...
          <div class="service-card-info">
            <h3 class="service-card-title">RF Database - Open Data</h3>
            <p class="service-card-subtitle">Search by city + activity in Receita Federal data</p>
          </div>
          <div id="rf-status"></div>
          <span class="service-card-badge badge-federal">Open Data</span>
        </div>
        <div class="service-card-body">
          <div style="display:grid;grid-template-columns:1fr 1fr auto;gap:1rem;align-items:end;">
            ...
            <div class="form-group">
              <label>CNAE (optional)</label>
              <input type="text" id="rf-cnae" placeholder="Ex: 5611-2 (restaurants)">
            </div>
      ...
      <div class="filters-bar">
        <div class="search-box">
          ...
          <input type="text" placeholder="Filter leads found..." oninput="LeadsPage.filterLocal(this.value)">
        </div>
        <div class="filters-actions">
          <span id="mine-count" style="color:var(--text-secondary);font-size:0.85rem;"></span>
          <button class="btn btn-sm btn-secondary" onclick="LeadsPage.exportCSV()" title="Export leads found"><i data-lucide="download"></i>CSV</button>
          <button class="btn btn-sm btn-secondary" onclick="LeadsPage.exportSocios()" title="Export real partners"><i data-lucide="users"></i>Partners</button>
          <button class="btn btn-sm btn-primary" onclick="LeadsPage.saveAll()"><i data-lucide="save"></i>Save All</button>
          <button class="btn btn-sm" id="btn-script-ligacao" onclick="LeadsPage.showScripts()" style="background:linear-gradient(135deg,#f59e0b,#d97706);color:white;padding:6px 12px;border-radius:var(--border-radius-sm);font-size:0.8rem;font-weight:600;cursor:pointer;border:none;"><i data-lucide="phone"></i>Call Script</button>
        </div>
      </div>
      ...
      <div class="empty-state" id="leads-empty">
        ...
        <h3 style="color:white;margin-top:1rem;">Ready to mine</h3>
        <p style="color:var(--text-tertiary);">Enter company type and city to search real data</p>
      </div>
    `;
    lucide.createIcons();
    this.currentLeads = [];
  ...
  async mine(e) {
    ...
    btn.disabled = true;
    btn.innerHTML = '<i data-lucide="loader-2" class="spin-animation"></i>Mining...';
    lucide.createIcons();
    ...
    try {
      ...
      document.getElementById('mine-count').textContent = `${this.currentLeads.length} leads found | Sources: ${(data.sources || []).join(', ')}`;
      showToast(`${this.currentLeads.length} leads successfully mined!`, 'success');
    } catch (err) {
      showToast('Mining error: ' + err.message, 'danger');
    }

    btn.disabled = false;
    btn.innerHTML = '<i data-lucide="radar"></i>Mine';
    lucide.createIcons();
  },
  ...
  async mineIndividuals(e) {
    ...
    btn.disabled = true;
    btn.innerHTML = '<i data-lucide="loader-2" class="spin-animation"></i>Mining...';
    status.innerHTML = '<p style="color:var(--accent-secondary);font-size:0.85rem;">Generating individual contacts...</p>';
    lucide.createIcons();
    ...
    try {
      ...
      this.renderResults(this.currentLeads);
      document.getElementById('mine-count').textContent = `${people.length} individuals found | ${data.saved || 0} saved to database | Category: ${this.pfCategory}`;
      status.innerHTML = `<div style="display:flex;gap:0.75rem;margin-top:0.5rem;flex-wrap:wrap;">
        <span style="background:rgba(236,72,153,0.15);color:#ec4899;padding:4px 12px;border-radius:20px;font-size:0.8rem;font-weight:600;">${data.saved || 0} saved automatically</span>
        <span style="background:rgba(129,140,248,0.15);color:#818cf8;padding:4px 12px;border-radius:20px;font-size:0.8rem;">${people.length} contacts generated</span>
      </div>`;
      showToast(`${people.length} individuals mined for ${city}!`, 'success');
    } catch (err) {
      status.innerHTML = `<p style="color:#f43f5e;font-size:0.85rem;">Error: ${err.message}</p>`;
      showToast('Error mining individuals: ' + err.message, 'danger');
    }

    btn.disabled = false;
    btn.innerHTML = '<i data-lucide="user-plus"></i>Mine Individuals';
    lucide.createIcons();
  },
  ...
  async minePeople() {
    ...
    if (!raw) { showToast('Paste at least one CNPJ', 'warning'); return; }

    const cnpjs = raw.split(/[\n,;]+/).map(c => c.replace(/\D/g, '')).filter(c => c.length === 14);
    if (cnpjs.length === 0) { showToast('No valid CNPJ found', 'warning'); return; }
    ...
    btn.disabled = true;
    btn.innerHTML = '<i data-lucide="loader-2" class="spin-animation"></i>Looking up...';
    countEl.textContent = `Looking up ${cnpjs.length} CNPJs in Receita Federal...`;
    resultDiv.innerHTML = '<div style="display:flex;align-items:center;gap:0.75rem;padding:1rem;"><div class="spin-animation" style="width:20px;height:20px;border:3px solid rgba(16,185,129,0.2);border-top-color:#10b981;border-radius:50%;"></div><span style="color:var(--text-secondary);font-size:0.85rem;">Fetching real data from Receita Federal...</span></div>';
    lucide.createIcons();
    ...
    try {
      ...
      if (people.length === 0) {
        resultDiv.innerHTML = '<p style="color:var(--text-tertiary);text-align:center;padding:1.5rem;">No people found for the provided CNPJs.</p>';
        countEl.textContent = '';
        btn.disabled = false;
        btn.innerHTML = '<i data-lucide="radar"></i>Extract Real People';
        lucide.createIcons();
        return;
      }

      countEl.textContent = `${people.length} real people found in ${cnpjs.length} CNPJs`;
          ...
          ${people.map(p => `
            <div style="background:rgba(16,185,129,0.04);border:1px solid rgba(16,185,129,0.12);border-radius:var(--border-radius-md);overflow:hidden;">
              <div style="padding:1rem 1.25rem;border-bottom:1px solid rgba(16,185,129,0.08);display:flex;justify-content:space-between;align-items:start;">
                ...
                <span style="display:inline-flex;align-items:center;gap:4px;background:rgba(16,185,129,0.12);color:#10b981;padding:3px 8px;border-radius:12px;font-size:0.65rem;font-weight:600;border:1px solid rgba(16,185,129,0.2);">REAL DATA</span>
              </div>
              <div style="padding:0.75rem 1.25rem;">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:0.4rem;font-size:0.78rem;">
                  ...
                  <div style="display:flex;align-items:center;gap:0.3rem;"><span style="color:var(--text-tertiary);">Activity:</span><span style="color:var(--text-secondary);">${p.atividade || 'N/A'}</span></div>
              ...
              <div style="padding:0.6rem 1.25rem;background:rgba(0,0,0,0.15);display:flex;justify-content:space-between;align-items:center;">
                ...
                <button class="btn btn-sm btn-primary" onclick='LeadsPage.savePersonLead(${JSON.stringify(p).replace(/'/g, "\\'")})' style="display:inline-flex;align-items:center;gap:4px;font-size:0.75rem;">
                  <i data-lucide="plus" style="width:12px;height:12px;"></i>Save
                </button>
              </div>
            </div>
          `).join('')}
      ...
      `;
      lucide.createIcons();
      showToast(`${people.length} real people extracted successfully!`, 'success');
    } catch (err) {
  ...
  savePersonLead(p) {
    ...
    this.currentLeads.unshift({ ...lead, id: p.id, fonte: 'Receita Federal (Real Partner)', score: 95 });
    this.renderResults(this.currentLeads);
    showToast(`${p.nome} saved as lead!`, 'success');
  },
  ...
  async checkRFStatus() {
    try {
      ...
      if (!el) return;
      if (data.available) {
        el.innerHTML = `<span style="display:inline-flex;align-items:center;gap:4px;background:rgba(16,185,129,0.12);color:#10b981;padding:4px 10px;border-radius:20px;font-size:0.7rem;font-weight:600;border:1px solid rgba(16,185,129,0.2);"><span style="width:6px;height:6px;border-radius:50%;background:#10b981;"></span>${data.total} CNPJs indexed</span>`;
      } else {
        el.innerHTML = `<span style="display:inline-flex;align-items:center;gap:4px;background:rgba(245,158,11,0.12);color:#f59e0b;padding:4px 10px;border-radius:20px;font-size:0.7rem;font-weight:600;border:1px solid rgba(245,158,11,0.2);">Index not configured</span>`;
      }
    } catch {}
  },
  ...
  async searchRF() {
    ...
    if (!city) { showToast('Enter a city', 'warning'); return; }

    const btn = document.getElementById('btn-rf-search');
    const resultDiv = document.getElementById('rf-results');
    btn.disabled = true;
    btn.innerHTML = '<i data-lucide="loader-2" class="spin-animation"></i>Searching...';
    lucide.createIcons();
    ...
    try {
        ...
        resultDiv.innerHTML = `
          <div style="padding:1.25rem;background:rgba(245,158,11,0.06);border:1px solid rgba(245,158,11,0.15);border-radius:var(--border-radius-md);">
            <div style="display:flex;align-items:center;gap:0.75rem;margin-bottom:0.75rem;">
              ...
              <span style="color:#f59e0b;font-weight:600;font-size:0.9rem;">RF Index not configured</span>
            </div>
            <p style="color:var(--text-secondary);font-size:0.82rem;margin:0 0 0.75rem;">To use search by Receita Federal open data, you need to download and process the CSVs.</p>
            <div style="background:rgba(0,0,0,0.2);border-radius:8px;padding:0.75rem;font-family:monospace;font-size:0.75rem;color:var(--text-secondary);">
              <p style="margin:0 0 4px;color:var(--text-tertiary);">// Step 1: Download data</p>
              <p style="margin:0 0 4px;">Go to: dadosabertos.rfb.gov.br/CNPJ/</p>
              <p style="margin:0 0 4px;">Download: Estabelecimentos*.zip</p>
              <p style="margin:0 0 8px;color:var(--text-tertiary);">// Step 2: Process</p>
              <p style="margin:0 0 4px;">Extract to: server/data/rf/</p>
      ...
      if (data.results.length === 0) {
        resultDiv.innerHTML = '<p style="color:var(--text-tertiary);text-align:center;padding:1rem;">No results found for this search.</p>';
        btn.disabled = false;
        btn.innerHTML = '<i data-lucide="search"></i>Search';
        lucide.createIcons();
        return;
      ...
      resultDiv.innerHTML = `
        <div style="margin-bottom:0.75rem;display:flex;justify-content:space-between;align-items:center;">
          <span style="color:var(--text-secondary);font-size:0.82rem;">${data.results.length} companies found in RF database</span>
          <button class="btn btn-sm btn-primary" onclick="LeadsPage.enrichRFResults()" id="btn-enrich-rf" style="display:inline-flex;align-items:center;gap:4px;">
            <i data-lucide="sparkles"></i>Enrich with Receita Federal
          </button>
        </div>
          ...
          ${data.results.map(r => `
            <div style="background:rgba(245,158,11,0.04);border:1px solid rgba(245,158,11,0.1);border-radius:var(--border-radius-sm);padding:0.85rem;" data-cnpj="${r.cnpj.replace(/\D/g, '')}">
              <div style="display:flex;justify-content:space-between;align-items:start;margin-bottom:0.4rem;">
                <h4 style="color:white;font-size:0.88rem;font-weight:600;margin:0;flex:1;">${escapeHtml(r.nome || 'No name')}</h4>
  ...
  async enrichRFResults() {
    ...
    if (cnpjs.length === 0) { showToast('Select at least 1 CNPJ', 'warning'); return; }

    const btn = document.getElementById('btn-enrich-rf');
    const resultDiv = document.getElementById('rf-results');
    btn.disabled = true;
    btn.innerHTML = '<i data-lucide="loader-2" class="spin-animation"></i>Enriching...';
    lucide.createIcons();

    try {
      const data = await API.post('/rfsearch/enrich', { cnpjs });
      if (data.results.length === 0) {
        showToast('No data found in Receita Federal', 'warning');
        btn.disabled = false;
        btn.innerHTML = '<i data-lucide="sparkles"></i>Enrich with Receita Federal';
        lucide.createIcons();
        return;
      }

      // Show enriched results
      let html = `<div style="margin-bottom:0.75rem;"><span style="color:#10b981;font-weight:600;font-size:0.85rem;">${data.results.length} companies enriched with real RF data</span></div>`;
        ...
        html += `
          <div style="background:rgba(16,185,129,0.04);border:1px solid rgba(16,185,129,0.12);border-radius:var(--border-radius-md);overflow:hidden;">
            <div style="padding:1rem 1.25rem;border-bottom:1px solid rgba(16,185,129,0.08);display:flex;justify-content:space-between;align-items:start;">
              ...
              <span style="background:rgba(16,185,129,0.12);color:#10b981;padding:3px 8px;border-radius:12px;font-size:0.65rem;font-weight:600;">REAL RF</span>
            </div>
            <div style="padding:0.75rem 1.25rem;font-size:0.78rem;">
              ...
              ${d.socios?.length ? `<div style="margin-top:0.5rem;"><p style="color:var(--text-tertiary);font-size:0.7rem;margin:0 0 4px;">REAL PARTNERS:</p>${d.socios.map(s => `<span style="display:inline-block;background:rgba(129,140,248,0.1);color:#818cf8;padding:2px 8px;border-radius:12px;font-size:0.7rem;margin:2px;">${escapeHtml(s.nome)} (${escapeHtml(s.qualificacao)})</span>`).join('')}</div>` : ''}
            </div>
            <div style="padding:0.5rem 1.25rem;background:rgba(0,0,0,0.15);display:flex;justify-content:flex-end;">
              <button class="btn btn-sm btn-primary" onclick='LeadsPage.saveCNPJLead(${JSON.stringify(d).replace(/'/g, "\\'")})' style="display:inline-flex;align-items:center;gap:4px;font-size:0.75rem;">
                <i data-lucide="plus" style="width:12px;height:12px;"></i>Save as Lead
              </button>
            </div>
          </div>`;
      }
      ...
      lucide.createIcons();
      showToast(`${data.results.length} companies enriched!`, 'success');
    } catch (err) {
      showToast('Error: ' + err.message, 'danger');
    }
  ...
  async lookupCNPJ() {
    ...
    resultDiv.innerHTML = '<p style="color:var(--accent-secondary);">Consulting Receita Federal...</p>';
      ...
      resultDiv.innerHTML = `
        <div style="background:rgba(16,185,129,0.06);border:1px solid rgba(16,185,129,0.15);border-radius:var(--border-radius-md);padding:1.25rem;">
          ...
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:0.75rem;font-size:0.82rem;">
            ...
            <div><span style="color:var(--text-tertiary);">Social Capital:</span><br><strong style="color:var(--accent-secondary);">R$ ${(d.capitalSocial || 0).toLocaleString('pt-BR')}</strong></div>
            <div><span style="color:var(--text-tertiary);">Size:</span><br><strong style="color:white;">${escapeHtml(d.porte || 'N/A')}</strong></div>
            <div><span style="color:var(--text-tertiary);">CNAE:</span><br><strong style="color:white;">${escapeHtml(d.cnaePrincipal || 'N/A')}</strong></div>
            <div><span style="color:var(--text-tertiary);">Regime:</span><br><strong style="color:white;">${escapeHtml(d.regimeTributario || 'N/A')}</strong></div>
            <div><span style="color:var(--text-tertiary);">Registration Date:</span><br><strong style="color:white;">${escapeHtml(d.dataAbertura || 'N/A')}</strong></div>
            <div><span style="color:var(--text-tertiary);">Phone:</span><br><strong style="color:white;">${escapeHtml(d.telefone1 || 'N/A')}</strong></div>
            <div><span style="color:var(--text-tertiary);">Simples Nacional:</span><br><strong style="color:white;">${d.opcaoSimples ? 'Yes' : 'No'}</strong></div>
            <div><span style="color:var(--text-tertiary);">MEI:</span><br><strong style="color:white;">${d.opcaoMEI ? 'Yes' : 'No'}</strong></div>
          </div>
          ...
          ${d.socios?.length ? `<div style="margin-top:0.75rem;"><p style="font-size:0.75rem;color:var(--text-tertiary);margin-bottom:0.3rem;">PARTNERS:</p>${d.socios.map(s => `<span style="display:inline-block;background:rgba(129,140,248,0.1);color:var(--accent-primary);padding:2px 8px;border-radius:12px;font-size:0.75rem;margin:2px;">${escapeHtml(s.nome)} (${escapeHtml(s.qualificacao)})</span>`).join('')}</div>` : ''}
          <button class="btn btn-sm btn-primary" style="margin-top:0.75rem;" onclick='LeadsPage.saveCNPJLead(${JSON.stringify(d).replace(/'/g,"\\'")})'><i data-lucide="plus"></i>Save as Lead</button>
        </div>
      `;
      lucide.createIcons();
    } catch (err) {
      resultDiv.innerHTML = `<p style="color:var(--danger);">CNPJ not found or lookup error.</p>`;
    }
  },
  ...
  async lookupCPF() {
    ...
    if (cpf.length !== 11) {
      showToast('CPF must contain 11 digits', 'warning');
      return;
    }
    ...
    resultDiv.innerHTML = `
      <div style="display:flex;align-items:center;gap:0.75rem;padding:1.5rem;background:rgba(34,211,238,0.04);border:1px solid rgba(34,211,238,0.1);border-radius:var(--border-radius-md);">
        ...
        <span style="color:var(--accent-secondary);font-size:0.9rem;">Consulting CPF <strong>${cpf.replace(/(\d{3})(\d{3})(\d{3})(\d{2})/, '$1.$2.$3-$4')}</strong> in Receita Federal...</span>
      </div>`;
    ...
    try {
      ...
      const validatedBadge = '<span style="display:inline-flex;align-items:center;gap:4px;background:rgba(16,185,129,0.12);color:#10b981;padding:4px 10px;border-radius:20px;font-size:0.7rem;font-weight:600;border:1px solid rgba(16,185,129,0.2);"><span style="width:6px;height:6px;border-radius:50%;background:#10b981;"></span>CPF Validated</span>';
        ...
        <div style="background:linear-gradient(135deg,rgba(34,211,238,0.06),rgba(99,102,241,0.04));border:1px solid rgba(34,211,238,0.12);border-radius:var(--border-radius-md);overflow:hidden;">
          ...
          <div style="padding:1.25rem 1.5rem;">
            <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:1rem;">
              ...
              <span style="font-size:0.75rem;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:0.05em;">Personal Data</span>
            </div>
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;margin-bottom:1.25rem;">
              <div>
                <p style="font-size:0.7rem;color:var(--text-tertiary);margin:0 0 4px;text-transform:uppercase;letter-spacing:0.03em;">Full Name</p>
                <p style="font-size:0.88rem;color:white;font-weight:600;margin:0;">${escapeHtml(d.nome)}</p>
              </div>
              <div>
                <p style="font-size:0.7rem;color:var(--text-tertiary);margin:0 0 4px;text-transform:uppercase;letter-spacing:0.03em;">Mother's Name</p>
                <p style="font-size:0.88rem;color:white;font-weight:600;margin:0;">${escapeHtml(d.nomeMae || 'Not informed')}</p>
              </div>
              <div>
                <p style="font-size:0.7rem;color:var(--text-tertiary);margin:0 0 4px;text-transform:uppercase;letter-spacing:0.03em;">Date of Birth</p>
                <p style="font-size:0.88rem;color:#22d3ee;font-weight:600;margin:0;">${escapeHtml(d.dataNascimento || 'Not informed')}</p>
              </div>
              <div>
                <p style="font-size:0.7rem;color:var(--text-tertiary);margin:0 0 4px;text-transform:uppercase;letter-spacing:0.03em;">Age</p>
                <p style="font-size:0.88rem;color:white;font-weight:600;margin:0;">${d.idade ? d.idade + ' years' : 'Not informed'}</p>
              </div>
            </div>
            <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:1rem;">
              <div>
                <p style="font-size:0.7rem;color:var(--text-tertiary);margin:0 0 4px;text-transform:uppercase;letter-spacing:0.03em;">Gender</p>
                <p style="font-size:0.88rem;color:white;font-weight:600;margin:0;">${escapeHtml(d.sexo || 'Not informed')}</p>
              </div>
              <div>
                <p style="font-size:0.7rem;color:var(--text-tertiary);margin:0 0 4px;text-transform:uppercase;letter-spacing:0.03em;">City</p>
                <p style="font-size:0.88rem;color:white;font-weight:600;margin:0;">${escapeHtml(d.municipio || 'Not informed')}</p>
              </div>
              <div>
                <p style="font-size:0.7rem;color:var(--text-tertiary);margin:0 0 4px;text-transform:uppercase;letter-spacing:0.03em;">State</p>
                <p style="font-size:0.88rem;color:white;font-weight:600;margin:0;">${escapeHtml(d.uf || 'Not informed')}</p>
              </div>
              <div>
                <p style="font-size:0.7rem;color:var(--text-tertiary);margin:0 0 4px;text-transform:uppercase;letter-spacing:0.03em;">Registration Status</p>
          ...
          <div style="padding:0 1.5rem 1.25rem;">
            <div style="display:flex;align-items:center;gap:0.5rem;margin-bottom:0.75rem;">
              ...
              <span style="font-size:0.75rem;font-weight:600;color:var(--text-tertiary);text-transform:uppercase;letter-spacing:0.05em;">Contact</span>
            </div>
            <div style="display:grid;grid-template-columns:repeat(2,1fr);gap:1rem;">
              ${d.telefone ? `<div><p style="font-size:0.7rem;color:var(--text-tertiary);margin:0 0 4px;text-transform:uppercase;">Phone</p><p style="font-size:0.88rem;color:#818cf8;font-weight:600;margin:0;">${escapeHtml(d.telefone)}</p></div>` : ''}
          ...
          <div style="padding:1rem 1.5rem;background:rgba(0,0,0,0.15);">
            <div style="display:flex;justify-content:space-between;align-items:center;">
              <div style="display:flex;align-items:center;gap:0.5rem;">
                ...
                <span style="font-size:0.72rem;color:var(--text-tertiary);">Real mathematical validation | Personal data: estimate based on CPF digits</span>
              </div>
              <button class="btn btn-sm btn-primary" onclick='LeadsPage.saveCPFLead(${JSON.stringify(d).replace(/'/g, "\\'")})' style="display:inline-flex;align-items:center;gap:6px;">
                <i data-lucide="plus" style="width:14px;height:14px;"></i>Save as Lead
              </button>
            </div>
            <p style="font-size:0.68rem;color:var(--text-tertiary);margin:6px 0 0;opacity:0.7;">Personal data of CPF are protected by LGPD. For 100% real data, use CNPJ Lookup above.</p>
          </div>
        </div>
      `;
      lucide.createIcons();
      ...
      resultDiv.innerHTML = `
        <div style="display:flex;align-items:center;gap:0.75rem;padding:1.25rem;background:rgba(244,63,94,0.06);border:1px solid rgba(244,63,94,0.15);border-radius:var(--border-radius-md);">
          ...
          <div>
            <p style="color:#f43f5e;font-weight:600;font-size:0.9rem;margin:0;">Invalid CPF or not found</p>
            <p style="color:var(--text-tertiary);font-size:0.8rem;margin:4px 0 0;">Check if CPF is correct and try again.</p>
          </div>
        </div>`;
      lucide.createIcons();
    }
  ...
  saveCNPJLead(d) {
    ...
    this.renderResults(this.currentLeads);
    showToast('Lead saved from Receita Federal!', 'success');
  },
  ...
  saveCPFLead(d) {
    ...
    this.currentLeads.unshift({ ...lead, id: 'cpf-' + Date.now(), fonte: 'CPF Lookup', score: 90 });
    this.renderResults(this.currentLeads);
    showToast('Lead saved from CPF lookup!', 'success');
  },
  ...
  renderResults(leads) {
    ...
    container.innerHTML = leads.map(l => {
      ...
      const sourceColors = { 'Receita Federal via BrasilAPI': '#10b981', 'OpenStreetMap Nominatim': '#38bdf8', 'Base local inteligente': '#a78bfa', 'Smart local database': '#a78bfa' };
      ...
      return `
        <div class="card" style="padding:1.25rem;position:relative;">
          ...
          <div style="display:flex;gap:0.4rem;flex-wrap:wrap;margin-bottom:0.5rem;">
            <span class="badge" style="background:${sourceColor}18;color:${sourceColor};border:1px solid ${sourceColor}33;">${escapeHtml(l.fonte || 'N/A')}</span>
          ...
          <div style="display:flex;justify-content:space-between;align-items:center;margin-top:0.75rem;padding-top:0.5rem;border-top:1px solid var(--border-color);">
            ...
            <button class="btn btn-sm btn-secondary" onclick="LeadsPage.saveToFunil('${l.id}')"><i data-lucide="plus"></i>Save</button>
          </div>
        </div>
      `;
    }).join('');
  ...
  async saveToFunil(id) {
    ...
    try {
      ...
      showToast(`"${lead.name}" saved to funnel!`, 'success');
    } catch (err) {
  ...
  async saveAll() {
    ...
    showToast(`${saved} leads saved to funnel!`, 'success');
  },
  ...
  async exportSocios() {
    showToast('Exporting real partners from Receita Federal...', 'info');
  ...
  async showScripts() {
    ...
    if (!keyword) {
      showToast('Search a niche first to see scripts', 'warning');
      return;
    }
    ...
    scriptBtn.innerHTML = '<i data-lucide="loader-2" class="spin-animation"></i>Loading...';
    ...
    try {
        ...
        <div style="background:linear-gradient(135deg,#1a1a2e,#16213e);border:1px solid rgba(245,158,11,0.2);border-radius:16px;max-width:800px;width:100%;max-height:80vh;overflow-y:auto;position:relative;">
          ...
          <div style="padding:1.5rem 2rem;border-bottom:1px solid rgba(245,158,11,0.1);display:flex;justify-content:space-between;align-items:center;position:sticky;top:0;background:linear-gradient(135deg,#1a1a2e,#16213e);z-index:1;">
            <div style="display:flex;align-items:center;gap:1rem;">
              ...
              <div>
                <h2 style="color:white;font-size:1.3rem;font-weight:700;margin:0;">Call Scripts</h2>
                <p style="color:var(--text-tertiary);font-size:0.85rem;margin:4px 0 0;">Niche: <strong style="color:#f59e0b;">${escapeHtml(keyword)}</strong> | App: <strong style="color:#10b981;">${escapeHtml(data.appName)}</strong></p>
              </div>
            </div>
              ...
              <div style="margin-bottom:1.5rem;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:12px;overflow:hidden;">
                ...
                <div style="padding:1.25rem;" id="script-content-${index}">
                  ...
                  <div style="margin-bottom:1rem;">
                    <p style="color:var(--text-tertiary);font-size:0.7rem;text-transform:uppercase;letter-spacing:0.05em;margin:0 0 6px;">OPENING</p>
                  ...
                  <!-- Body -->
                  <div style="margin-bottom:1rem;">
                    <p style="color:var(--text-tertiary);font-size:0.7rem;text-transform:uppercase;letter-spacing:0.05em;margin:0 0 6px;">CONTENT</p>
                  ...
                  <!-- Closing -->
                  <div>
                    <p style="color:var(--text-tertiary);font-size:0.7rem;text-transform:uppercase;letter-spacing:0.05em;margin:0 0 6px;">CLOSING</p>
          ...
          <div style="padding:1rem 2rem;border-top:1px solid rgba(255,255,255,0.06);display:flex;justify-content:space-between;align-items:center;">
            <p style="color:var(--text-tertiary);font-size:0.75rem;margin:0;">${data.scripts.length} scripts available | App: ${escapeHtml(data.appName)}</p>
            <button onclick="LeadsPage.closeScriptsModal()" style="background:rgba(255,255,255,0.1);border:1px solid rgba(255,255,255,0.15);color:white;padding:8px 16px;border-radius:8px;font-size:0.85rem;cursor:pointer;transition:all 0.2s;">Close</button>
          </div>
        </div>
      `;
      ...
      showToast('Error loading scripts: ' + err.message, 'danger');
    }
  ...
  copyScript(index) {
    ...
    paragraphs.forEach(p => {
      if (!p.textContent.match(/^(OPENING|CONTENT|CLOSING)$/)) {
        text += p.textContent + '\n\n';
      }
    });
