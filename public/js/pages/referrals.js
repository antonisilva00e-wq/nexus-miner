...
const ReferralsPage = {
  async render() {
    document.getElementById('page-title').textContent = 'Refer and Earn';
    document.getElementById('page-subtitle').textContent = 'Share your link and earn 40% on every sale';
    ...
    try {
      ...
      el.innerHTML = `
        ...
        <div class="card" style="margin-bottom:1.5rem;padding:2rem;background:linear-gradient(135deg,rgba(16,185,129,0.1),rgba(5,150,105,0.05));text-align:center;">
          <p style="font-size:0.8rem;color:var(--text-tertiary);margin:0;text-transform:uppercase;letter-spacing:0.1em;">Your Available Balance</p>
          <h1 style="color:#10b981;font-size:3rem;margin:0.5rem 0;font-weight:800;">R$ ${(balanceData.balance || 0).toFixed(2)}</h1>
          <p style="color:var(--text-secondary);font-size:0.9rem;margin:0;">Commission: <strong style="color:#10b981;">${balanceData.commission}%</strong> on every sale</p>
        </div>

        <!-- Link de convite -->
        <div class="card" style="margin-bottom:1.5rem;">
          <h3 style="color:white;font-size:1.1rem;margin:0 0 1rem;display:flex;align-items:center;gap:0.5rem;"><i data-lucide="link" style="color:var(--accent-primary);"></i>Your Invite Link</h3>
          <div style="background:rgba(0,0,0,0.2);border-radius:var(--border-radius-md);padding:1rem;display:flex;align-items:center;gap:1rem;">
            ...
            <button class="btn btn-sm btn-primary" onclick="ReferralsPage.copyLink()"><i data-lucide="copy"></i>Copy</button>
          </div>
        ...
        <!-- Como funciona -->
        <div class="card" style="margin-bottom:1.5rem;">
          <h3 style="color:white;font-size:1.1rem;margin:0 0 1rem;">How It Works</h3>
          <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:1.5rem;">
            <div style="text-align:center;">
              ...
              <h4 style="color:white;font-size:0.9rem;margin:0 0 0.3rem;">Copy the Link</h4>
              <p style="color:var(--text-tertiary);font-size:0.78rem;margin:0;">Copy your unique invite link</p>
            </div>
            <div style="text-align:center;">
              ...
              <h4 style="color:white;font-size:0.9rem;margin:0 0 0.3rem;">Share</h4>
              <p style="color:var(--text-tertiary);font-size:0.78rem;margin:0;">Send to friends and acquaintances</p>
            </div>
            <div style="text-align:center;">
              ...
              <h4 style="color:white;font-size:0.9rem;margin:0 0 0.3rem;">Earn 40%</h4>
              <p style="color:var(--text-tertiary);font-size:0.78rem;margin:0;">Receive 40% on every sale</p>
            </div>
          </div>
        </div>
        ...
        <!-- Historico -->
        <div class="card">
          <h3 style="color:white;font-size:1.1rem;margin:0 0 1rem;">Commission History</h3>
  ...
  async loadCommissions() {
    try {
      ...
      if (!data.commissions || data.commissions.length === 0) {
        el.innerHTML = '<p style="color:var(--text-tertiary);text-align:center;padding:1.5rem;">No commissions yet</p>';
        return;
      }
      el.innerHTML = data.commissions.map(c => `
        <div style="display:flex;justify-content:space-between;align-items:center;padding:0.75rem;border-bottom:1px solid var(--border-color);">
          <div>
            <span style="color:white;font-weight:500;">${c.referred_name || 'Client'}</span>
  ...
  shareWhatsApp() {
    ...
    window.open(`https://wa.me/?text=${encodeURIComponent('Check out Nexus Miner: ' + link)}`);
  },

  shareEmail() {
    const link = document.getElementById('invite-link').value;
    window.open(`mailto:?subject=Nexus Miner Invitation&body=${encodeURIComponent('Check out Nexus Miner: ' + link)}`);
  },
};
